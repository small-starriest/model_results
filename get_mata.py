...
def get_model_parameters_memory(model_info: ModelInfo) -> tuple[int| None, float|None]:
    ...
    logger.info(f"Could not find the results parameters for {model_info.id}")
    return None, None
...
def create_model_meta(model_info: ModelInfo) -> ModelMeta | None:
    ...
    try:
        ...
        logger.error(f"Error getting results parameters for {model_info.id}, {e}")
...
def parse_readme(model_info: ModelInfo) -> dict[str, dict[str, Any]] | None:
    ...
    meta = metadata_load(readme_path)
    if "results-index" not in meta:
        logger.info(f"Could not find results-index in {model_id}")
        return
    model_index = meta["results-index"][0]
    ...
    # There a lot of reuploads with tunes, quantization, etc. We only want the original results
    # to prevent this most of the time we can check if the results name from the readme is the same as the results id
...
def get_mteb_data() -> None:
    ...
    # models = [results for results in models if results.id == "intfloat/multilingual-e5-large"]
    for i, model_info in enumerate(models, start=1):
        #print("model::"+model_info.id)
        model_newName=model_info.id
        if '/' in model_newName:
            model_newName = model_newName.split('/', 1)[1]

            # 删除-instruct
        model_newName = model_newName.replace('-instruct', '')
        #print("model::" + model_newName)
        pd=False
        
        for dir_name in os.listdir('.'):
            dir_newName=dir_name
            if '__' in dir_newName:
                dir_newName = dir_newName.split('__', 1)[1]
            # 删除-instruct和+instruct
            dir_newName = dir_newName.replace('-instruct', '').replace('+instruct', '')
            #print("dirname::"+dir_newName)
            if(dir_newName==model_newName):
                pd=True
                fileName = dir_name
                break
        if(pd==False):
        ...
        model_path = Path(fileName) / "external"
        ...
        if model_info.id.lower().endswith("gguf"):
            logger.info(f"Skipping {model_info.id} GGUF results")
            continue
        ...
        if not model_meta:
            logger.warning(f"Could not get results meta or results for {model_info.id}")
            continue
